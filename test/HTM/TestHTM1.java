...
"</html>";
    ...
    public static void assertDQ(HTML that,StringBuffer buf,String[] txtDQ) {
        for (int i=0; i<that.DQArea.size(); i++) {
            String tmp=Main.ToSTR(that.DQArea.get(i), buf);
            assertTrue(tmp.equals(Main.DQ(txtDQ[i])));
        }
    }
    public static void assertTAG(HTML that,StringBuffer buf,String[] txtTag) {
...
"</body>" ,
"</html>"};
         assertDQ(that,BufHTM1,txtDQS);
         assertTAG(that,BufHTM1,txtTags);
        for (int j=0,i=1; i<that.LeftOrRight.size()-1; i+=2, j++) {
            int Start= that.LeftOrRight.get(i);
            int End=that.LeftOrRight.get(i+1);
            ...
            switch(j) {
                case 6:
                    assertTrue("scriptº–≈“".equals(tmp)); break;
                default:

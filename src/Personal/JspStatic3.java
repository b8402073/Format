...
public class JspStatic3  {
     ...
     * 是否顯示註解的編號
     */
    public boolean showCommentID=false;
    /**
     * *
            ...
            for (i = 0; i < MyFocus.size(); i++) {
                ...
                if (Eye.size() >= 2) {
                    ...
                    for (Pair P : comments) {
                        ...
                        ret.append(CommentToStr(P, Level,showCommentID));
                    }
                }
    ...
    public String CommentToStr(Pair p, int level,boolean showID) {
        ...
        if (that.startsWith("/*")) {
            ...
            for (int i = 0; i < sArr.length; i++) {
                sArr[i] = sArr[i].replace("\r\n", "");sArr[i] = sArr[i].replace("\r","");sArr[i] = sArr[i].replace("\n","");                
                String S="";
                if (i > 0 && i < sArr.length - 1) {                    
                    ...
                    String line = sArr[i].substring(2);
                    S+="/*";
                    if (showID) {
                        S += ID;
                    }
                    ...
                    //old code:  String line= sArr[i].replaceLast("*/", "");  //Please dont try to implement our own replaceLast function...it's hard...  
                    String line = sArr[i].substring(0, sArr[i].length()-2);  //We rewrite this line because the last two characters in this line must end with "*/"
                    if (showID) {
                        S+=ID;
                    }
                    S +=(line + "*/");
   ...
   /**
    ...
    public Vector<String> CollectVariableAboutWhat(String what) {
        //以下的程式碼是要尋找"PdfPTable"或"PdfPcell" 的相關變數
        Vector<String> ret=new Vector<String>();
        for (int x=0; x<MyFocus.size(); x++) {
            String that=MyFocus.get(x).RetString;
            if (that.equals(what)) {
                String[] WeDontCollect={what,"new","=","(",")","[","]",";" };                
                while(!MyFocus.get(x).RetString.equals(";")) {
                    if (!JspStatic3.In(WeDontCollect, MyFocus.get(x).RetString)  && !IsNumber(MyFocus.get(x).RetString)) {
                        ret.add(MyFocus.get(x).RetString);                                
                    }
                    x++;
                }                
            }
    ...
    public Vector<String> CollectVariableAboutSetPadding() {
        Vector<String> ret=new Vector<String>();
        for (int x=0; x<MyFocus.size(); x++) {
            String that=MyFocus.get(x).RetString;
            if (that.equals("setPadding")) {
                int H=SearchForStatementHeadPos(x,MyFocus);
                x=H;
                String[] WeDontCollect={"getDefaultCell","setPadding","(",")",";","."};                
                while(!MyFocus.get(x).RetString.equals(";")) {
                    if (!JspStatic3.In(WeDontCollect, MyFocus.get(x).RetString))
                        ret.add(MyFocus.get(x).RetString);
                    x++;
                }
            }
    ...
    public static boolean In(String[] arr,String that) {
